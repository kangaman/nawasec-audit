...
add_result() {
    ...
    if [[ "$OPT_QUIET" -eq 0 ]]; then
        ...
        [[ -n "$explanation" ]] && [[ "$status" =~ ^(WARN|FAIL)$ ]] && echo -e "    ${DIM}ℹ️ ${explanation}${N}"
        [[ -n "$recommendation" ]] && [[ "$status" =~ ^(WARN|FAIL)$ ]] && echo -e "    ${M}🔧 ${recommendation}${N}"
    fi
    local esc_msg=$(echo "$message" | sed 's/"/\\"/g')
    local esc_expl=$(echo "$explanation" | sed 's/"/\\"/g')
    local esc_rec=$(echo "$recommendation" | sed 's/"/\\"/g')
    RESULTS+=("{\"category\":\"${category}\",\"name\":\"${name}\",\"status\":\"${status}\",\"severity\":\"${severity}\",\"message\":\"${esc_msg}\",\"explanation\":\"${esc_expl}\",\"recommendation\":\"${esc_rec}\"}")
}
...
audit_cpanel_detection() {
    ...
    if [[ ! -d /usr/local/cpanel ]]; then
        add_result "detection" "cPanel" "FAIL" "CRITICAL" "cPanel not found" "This system does not have cPanel installed" "" "Install cPanel: https://docs.cpanel.net/installation-guide/" "" ""
        return 1
    fi
    add_result "detection" "cPanel" "PASS" "INFO" "Found at /usr/local/cpanel" "" "" "" "" ""

    local cpanel_version=$(cat /usr/local/cpanel/version 2>/dev/null || echo "unknown")
    add_result "detection" "cPanel Version" "INFO" "INFO" "$cpanel_version" "" "" "" "" ""

    if systemctl is-active --quiet cpanel 2>/dev/null || /usr/local/cpanel/cpsrvd status 2>/dev/null | grep -q "running"; then
        add_result "detection" "cpsrvd" "PASS" "INFO" "Running" "" "" "" "" ""
    else
        add_result "detection" "cpsrvd" "WARN" "HIGH" "Not running" "cPanel service is not active" "" "Start: /usr/local/cpanel/startup" "" ""
    fi

    # WHM access
    local whm_port=$(ss -tlnp 2>/dev/null | grep -c ":2087" || echo "0")
    if [[ "$whm_port" -gt 0 ]]; then
        add_result "detection" "WHM Port" "PASS" "INFO" "2087 open" "" "" "" "" ""
    fi
...
audit_whm_security() {
    ...
    if [[ -f /var/cpanel/authn/api_tokens/root/cpanel.json ]]; then
        add_result "whm" "API Tokens" "INFO" "INFO" "Root API tokens exist" "" "" "" "" ""
    fi
    ...
    if [[ -f "$tweak_conf" ]]; then
        ...
        if [[ "$require_ssl" == "1" ]]; then
            add_result "whm" "Require SSL" "PASS" "HIGH" "Enabled — WHM requires HTTPS" "" "" "" "" ""
        else
            add_result "whm" "Require SSL" "FAIL" "HIGH" "Disabled — WHM accessible via HTTP" \
        ...
        if [[ "$refsec" == "1" ]]; then
            add_result "whm" "Referrer Security" "PASS" "MEDIUM" "Enabled" "" "" "" "" ""
        else
            add_result "whm" "Referrer Security" "WARN" "MEDIUM" "Disabled" \
        ...
        if [[ -n "$max_emails" ]] && [[ "$max_emails" -gt 0 ]] && [[ "$max_emails" -le 500 ]]; then
            add_result "whm" "Max Emails/Hour" "PASS" "MEDIUM" "$max_emails" "" "" "" "" ""
        elif [[ -n "$max_emails" ]]; then
            add_result "whm" "Max Emails/Hour" "WARN" "MEDIUM" "$max_emails — may be too high" \
...
audit_security_config() {
    ...
    if [[ "$cphulk" == "1" ]]; then
        add_result "security" "cPHulk" "PASS" "HIGH" "Enabled — brute force protection active" "" "" "" "" ""
    else
        add_result "security" "cPHulk" "FAIL" "HIGH" "Disabled" \
    ...
    if [[ "$ssh_pass" == "no" ]]; then
        add_result "security" "SSH Password Auth" "PASS" "HIGH" "Disabled — key-only authentication" "" "" "" "" ""
    else
        add_result "security" "SSH Password Auth" "WARN" "HIGH" "Enabled" \
    ...
    if [[ -n "$shell_access" ]]; then
        add_result "security" "Default Shell" "INFO" "INFO" "$shell_access" "" "" "" "" ""
    fi
    ...
    if [[ -x /usr/bin/gcc ]]; then
        ...
        if [[ "$gcc_perms" == "700" ]] || [[ "$gcc_perms" == "750" ]]; then
            add_result "security" "Compiler Access" "PASS" "MEDIUM" "Restricted ($gcc_perms)" "" "" "" "" ""
        else
            add_result "security" "Compiler Access" "WARN" "MEDIUM" "Available to all users ($gcc_perms)" \
...
audit_php_config() {
    ...
    if [[ "$php_versions" -gt 0 ]]; then
        add_result "php" "PHP Versions" "INFO" "INFO" "$php_versions versions installed" "" "" "" "" ""
    fi
    ...
    for php_bin in /opt/cpanel/ea-php*/root/usr/bin/php; do
        ...
        if [[ "$major" -lt 7 ]] || [[ "$major" -eq 7 && "$minor" -lt 4 ]]; then
            add_result "php" "PHP $ver" "FAIL" "HIGH" "End of Life — no security patches" \
    ...
    if [[ -n "$disable_func" ]] && [[ "$disable_func" != "no value" ]]; then
        add_result "php" "disable_functions" "PASS" "HIGH" "Configured" "" "" "" "" ""
    else
        add_result "php" "disable_functions" "FAIL" "HIGH" "Not configured" \
    ...
    if [[ "$expose_php" == "0" ]] || [[ "$expose_php" == "" ]]; then
        add_result "php" "expose_php" "PASS" "LOW" "Off — PHP version hidden" "" "" "" "" ""
    else
        add_result "php" "expose_php" "WARN" "LOW" "On — PHP version exposed in headers" \
    ...
    if [[ "$display_errors" == "0" ]] || [[ "$display_errors" == "" ]]; then
        add_result "php" "display_errors" "PASS" "MEDIUM" "Off — errors not shown to users" "" "" "" "" ""
    else
        add_result "php" "display_errors" "FAIL" "MEDIUM" "On — errors visible to users" \
...
audit_email_security() {
    ...
    if [[ -n "$spf" ]]; then
        add_result "email" "SPF" "PASS" "MEDIUM" "Configured" "" "" "" "" ""
    else
        add_result "email" "SPF" "WARN" "MEDIUM" "Not detected in config" \
    ...
    if [[ -d /var/cpanel/domain_keys ]]; then
        ...
        if [[ "$dkim_count" -gt 0 ]]; then
            add_result "email" "DKIM" "PASS" "MEDIUM" "$dkim_count domains configured" "" "" "" "" ""
        else
            add_result "email" "DKIM" "WARN" "MEDIUM" "No DKIM keys found" \
    ...
    if [[ -f /etc/exim.conf ]]; then
        ...
        if [[ -n "$smtp_restrict" ]]; then
            add_result "email" "SMTP Restrictions" "PASS" "MEDIUM" "Configured" "" "" "" "" ""
        fi
    fi
}
...
audit_backup_config() {
    ...
    if [[ -f "$backup_conf" ]]; then
        ...
        if [[ "$backup_enabled" == "yes" ]]; then
            add_result "backup" "Backup" "PASS" "HIGH" "Enabled" "" "" "" "" ""
            ...
            add_result "backup" "Backup" "FAIL" "HIGH" "Disabled" \
        ...
        add_result "backup" "Backup" "WARN" "HIGH" "Configuration not found" \
...
audit_firewall() {
    ...
    if [[ -f /etc/csf/csf.conf ]]; then
        ...
        if [[ "$csf_enabled" == "0" ]]; then
            add_result "firewall" "CSF" "PASS" "HIGH" "Active (TESTING=0)" "" "" "" "" ""
        else
            add_result "firewall" "CSF" "WARN" "HIGH" "In TESTING mode" \
        ...
        add_result "firewall" "CSF" "WARN" "HIGH" "Not installed" \
    ...
    if [[ -f /etc/apache2/conf.d/modsec2.conf ]] || [[ -f /etc/nginx/modsec/modsec2.conf ]]; then
        add_result "firewall" "ModSecurity" "PASS" "HIGH" "Installed" "" "" "" "" ""
    else
        add_result "firewall" "ModSecurity" "WARN" "HIGH" "Not detected" \
